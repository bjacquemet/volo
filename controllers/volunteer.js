                ...
                ActivityController.getVolunteerSkills(req.params.id, function (err, skills) {
                    if (err) console.log(err);
                        ...
                        RecommendationController.getRecofromVolunteerId(req.params.id, function (err, reco) {
                            ...
                            res.render('volunteer/profile', {
                                title: volunteer.first_name + ' ' + volunteer.last_name + '\'s VOLO profile',
                                user: req.user,
                                volunteer: volunteer,
...
exports.searchProfile = function (req, res) {
    if (req.query.search && req.query.search.length > 3) {
            ...
            function (err, results) {
                ...
                if (result_length == 0) {
                    res.render('volunteer/search', {
                        title: "Volunteer Results for search " + req.query.search,
                        user: req.user,
                        results: null
                    })
                }
        ...
        res.render('volunteer/search', {
            title: "No Volunteer Results for search " + req.query.search,
            user: req.user
        });
    }
};
